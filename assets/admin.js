/* Layrix Admin UI — built by esbuild, do not edit assets/admin.js directly */
(() => {
  // src/js/modules/ui-widgets.js
  function initUiWidgets($, ctx) {
    ...
    (function() {
      ...
      try {
        ...
        if (!stored) return;
        var $bar = $('<div class="ecf-undo-bar" role="status"><span class="ecf-undo-bar__msg">Preset angewendet.</span><button type="button" class="ecf-btn ecf-btn--ghost ecf-btn--sm ecf-undo-bar__action">R\xFCckg\xE4ngig</button><button type="button" class="ecf-undo-bar__dismiss" aria-label="Schlie\xDFen">\xD7</button></div>');
        ...
        $bar.find(".ecf-undo-bar__action").on("click", function() {
          try {
            ...
            window.fetch(restUrl, {
              ...
              $bar.find(".ecf-undo-bar__action").prop("disabled", false).text("R\xFCckg\xE4ngig");
            });
          } catch (e) {
          }
        });
    ...
    function checkEmptyTokenGroups() {
      var groups = [
        { selector: '[data-group="colors"] .ecf-row--color', container: '[data-group="colors"]', msg: "Noch keine Farben. F\xFCge deine erste Farbe hinzu oder wende ein Stil-Preset an." },
        { selector: '[data-group="radius"] .ecf-row--minmax', container: '[data-group="radius"]', msg: "Noch keine Radien. F\xFCge deinen ersten Radius-Token hinzu." },
        { selector: '[data-group="shadows"] .ecf-row', container: '[data-group="shadows"]', msg: "Noch keine Schatten. F\xFCge deinen ersten Shadow-Token hinzu." }
      ];
    ...
    var wizardSteps = [
      {
        ...
        body: "Layrix verwaltet deine Design-Tokens zentral und \xFCbertr\xE4gt sie mit einem Klick in Elementor. Dieser kurze Guide zeigt dir alle Bereiche.",
        next: "Los geht's"
      },
      {
        mode: "callout",
      ...
      },
      {
        ...
        panel: "typography",
        title: "Typografie",
        body: "Schriftarten, Schriftgr\xF6\xDFen und Typografie-Skala. Lege fest welche Fonts auf der Webseite verwendet werden und wie gro\xDF \xDCberschriften im Verh\xE4ltnis zum Flie\xDFtext sind.",
        next: "Weiter"
      },
      {
        mode: "callout",
        panel: "spacing",
        title: "Abst\xE4nde",
        body: "Definiere dein Spacing-System \u2014 die Basis-Abst\xE4nde und den Rhythmus zwischen Elementen. Layrix generiert daraus eine vollst\xE4ndige Skala.",
        next: "Weiter"
      },
      {
        mode: "callout",
        panel: "shadows",
        title: "Schatten",
        body: "Erstelle wiederverwendbare Schatten-Tokens. Von subtilen Elevation-Effekten bis zu ausgepr\xE4gten Drop Shadows.",
        next: "Weiter"
      },
      {
        mode: "callout",
        panel: "variables",
        title: "Variablen",
        body: "Eigene CSS-Variablen die \xFCber Elementor hinaus gelten \u2014 z.B. f\xFCr Animationszeiten, Z-Index-Stufen oder andere Design-Werte.",
        next: "Weiter"
      },
      {
        mode: "callout",
        panel: "utilities",
        title: "Klassen",
        body: "Globale Elementor-Klassen f\xFCr wiederkehrende Stile. W\xE4hle aus der Bibliothek oder erstelle eigene Klassen die in jedem Widget verf\xFCgbar sind.",
        next: "Weiter zu Sync"
      },
      {
        mode: "callout",
        ...
        body: "Wenn alle Tokens eingerichtet sind, klicke auf <strong>Native Elementor Sync</strong>. Alle Tokens stehen danach als CSS-Variablen direkt im Elementor-Editor zur Verf\xFCgung.",
        next: "Fertig"
      },
      {
        mode: "toast",
    ...
    var $wizardNavPulse = null;
    function wizardDots(activeIdx) {
      return wizardSteps.filter(function(s) {
        return s.mode !== "toast";
      }).map(function(_, i) {
    ...
    function closeWizardCallout() {
      if ($wizardCallout) {
        $wizardCallout.remove();
        $wizardCallout = null;
      }
      if ($wizardNavPulse) {
        $wizardNavPulse.removeClass("ecf-wizard-nav-pulse");
        $wizardNavPulse = null;
      }
    ...
    function showWizardStep(idx) {
      ...
      if (step.mode === "modal") {
        ...
        var $modal = $('<div class="ecf-wizard"><button type="button" class="ecf-wizard__close" aria-label="Schlie\xDFen">\xD7</button><div class="ecf-wizard__steps">' + wizardDots(idx) + '</div><div class="ecf-wizard__title">' + $("<span>").text(step.title).html() + '</div><div class="ecf-wizard__body">' + $("<span>").text(step.body).html() + '</div><div class="ecf-wizard__footer"><button type="button" class="ecf-btn ecf-btn--secondary ecf-wizard__next">' + $("<span>").text(step.next).html() + "</button></div></div>");
        ...
        $modal.find(".ecf-wizard__close").on("click", function() {
          try {
            window.sessionStorage.setItem(wizardKey, "1");
          } catch (e) {
          }
          $overlay.remove();
        });
        ...
        $wizardCallout = $('<div class="ecf-wizard-callout" role="dialog" aria-label="Wizard"><div class="ecf-wizard-callout__arrow"></div><div class="ecf-wizard-callout__steps">' + wizardDots(idx) + '</div><strong class="ecf-wizard-callout__title">' + $("<span>").text(step.title).html() + '</strong><p class="ecf-wizard-callout__body">' + step.body + '</p><div class="ecf-wizard-callout__footer"><button type="button" class="ecf-btn ecf-btn--ghost ecf-btn--sm ecf-wizard-callout__skip">\xDCberspringen</button><button type="button" class="ecf-btn ecf-btn--secondary ecf-btn--sm ecf-wizard-callout__next">' + $("<span>").text(step.next).html() + "</button></div></div>");
        ...
        var $toast = $('<div class="ecf-wizard-toast"><span class="ecf-wizard-toast__icon">\u{1F389}</span><div><strong>' + $("<span>").text(step.title).html() + "</strong><span>" + $("<span>").text(step.body).html() + '</span></div><button type="button" class="ecf-wizard-toast__close" aria-label="Schlie\xDFen">\xD7</button></div>');
        $("body").append($toast);
        setTimeout(function() {
    ...
    try {
      if (!window.sessionStorage.getItem(wizardKey)) showWizard();
    } catch (e) {
    }
    ...
    $(document).on("submit", ".ecf-health-fix-form", function(e) {
      ...
      var classes = [];
      try {
        classes = JSON.parse($form.attr("data-ecf-confirm-classes") || "[]");
      } catch (ex) {
      }
      var classHtml = "";
      if (classes.length > 0) {
        classHtml = '<ul class="ecf-confirm-modal__list">' + classes.map(function(c) {
      ...
      var $modal = $('<div class="ecf-confirm-modal" role="dialog" aria-modal="true"><div class="ecf-confirm-modal__icon">\u{1F5D1}\uFE0F</div><h3 class="ecf-confirm-modal__title">' + $("<span>").text(classes.length === 1 ? "Klasse entfernen?" : "Klassen entfernen?").html() + '</h3><p class="ecf-confirm-modal__body">' + (classes.length === 1 ? "Diese Klasse wird aus Elementor entfernt. Sie kann danach neu synchronisiert werden." : "Diese " + classes.length + " Klassen werden aus Elementor entfernt. Sie k\xF6nnen danach neu synchronisiert werden.") + "</p>" + classHtml + '<div class="ecf-confirm-modal__footer"><button type="button" class="ecf-btn ecf-btn--ghost ecf-confirm-modal__cancel">Abbrechen</button><button type="button" class="ecf-btn ecf-btn--danger ecf-confirm-modal__confirm">' + $("<span>").text(title).html() + "</button></div></div>");
      $overlay.append($modal);
      $("body").append($overlay);
      setTimeout(function() {
      ...
      $overlay.on("click", function(ev) {
        if ($(ev.target).is($overlay)) closeModal();
      });
      $(document).one("keydown.ecfConfirm", function(ev) {
  ...
  jQuery(function($) {
    ...
    $(document).on("click", ".ecf-add-local-font", function() {
      ...
      var html = '<div class="ecf-font-file-row"><input type="text" data-ecf-slug-field="token" name="' + key + "[" + index + '][name]" value="" placeholder="' + escapeHtml(i18n.local_font_name_placeholder || "") + '" /><input type="text" name="' + key + "[" + index + '][family]" value="" placeholder="' + escapeHtml(i18n.local_font_family_placeholder || "") + '" /><div class="ecf-font-file-picker"><input type="text" class="ecf-font-file-url" name="' + key + "[" + index + '][src]" value="" placeholder="' + escapeHtml(i18n.local_font_upload_placeholder || "") + '" readonly /><button type="button" class="button ecf-font-file-select">' + ecfAdmin.i18n.select_file + '</button></div><input type="text" name="' + key + "[" + index + '][weight]" value="400" placeholder="400" /><select name="' + key + "[" + index + '][style]">' + styleOptions + '</select><select name="' + key + "[" + index + '][display]">' + displayOptions + '</select><button type="button" class="button ecf-remove-row">\xD7</button></div>';
      $table.append($(html));
      scheduleSettingsAutosave({ delay: 250 });
    });
    ...
    function renderFontFamilyGroupsIntoSelect($select, groups, current) {
      ...
      groups.forEach(function(group) {
        ...
        groupOptions.forEach(function(option) {
          options.push(option);
          $("<option>").attr("value", option.value).attr("data-ecf-font-source", option.source || "").text(option.label).appendTo($group);
        });
    ...
    function buildLocalFontRowHtml(inputKey2, index, row) {
      ...
      return '<div class="ecf-font-file-row"><input type="text" data-ecf-slug-field="token" name="' + inputKey2 + "[" + index + '][name]" value="' + escapeHtml(row && row.name || "") + '" placeholder="' + escapeHtml(i18n.local_font_name_placeholder || "") + '" /><input type="text" name="' + inputKey2 + "[" + index + '][family]" value="' + escapeHtml(row && row.family || "") + '" placeholder="' + escapeHtml(i18n.local_font_family_placeholder || "") + '" /><div class="ecf-font-file-picker"><input type="text" class="ecf-font-file-url" name="' + inputKey2 + "[" + index + '][src]" value="' + escapeHtml(row && row.src || "") + '" placeholder="' + escapeHtml(i18n.local_font_upload_placeholder || "") + '" readonly /><button type="button" class="button ecf-font-file-select">' + escapeHtml(i18n.select_file || "") + '</button></div><input type="text" name="' + inputKey2 + "[" + index + '][weight]" value="' + escapeHtml(row && row.weight || "400") + '" placeholder="400" /><select name="' + inputKey2 + "[" + index + '][style]">' + styleOptions + '</select><select name="' + inputKey2 + "[" + index + '][display]">' + displayOptions + '</select><button type="button" class="button ecf-remove-row">\xD7</button></div>';
    }
    function syncLocalFontRowsFromSettings(settings) {
      ...
      $("[data-local-font-table]").each(function() {
        var $table = $(this);
        var inputKey2 = String($table.data("input-key") || "");
        $table.find(".ecf-font-file-row").remove();
        rows.forEach(function(row, index) {
          $table.append($(buildLocalFontRowHtml(inputKey2, index, row)));
        });
      });
    }
    ...
    function syncBodySizeWithTypeScale(force) {
      ...
      $("[data-ecf-body-size-field]").each(function() {
        ...
        var currentValue = $.trim(String($field.find("[data-ecf-size-value-input]").val() || ""));
        var currentFormat = $.trim(String($field.find("[data-ecf-size-format-input], [data-ecf-format-input]").first().val() || "")).toLowerCase();
        var matchesPreviousDerived = previousDerived && currentFormat === previousDerived.format && currentValue === previousDerived.value;
    ...
    function syncGeneralFavoriteTogglesFromSettings(settings) {
      var favorites = settings && settings.general_setting_favorites && typeof settings.general_setting_favorites === "object" ? settings.general_setting_favorites : {};
    ...
    function updateFontPairingCard($card, pairing) {
      ...
      $card.find("[data-ecf-font-pairing-preview-head]").text(headingSample).css("font-family", "'" + headingFamily + "', serif");
      $card.find("[data-ecf-font-pairing-preview-body]").text(bodySample).css("font-family", "'" + bodyFamily + "', sans-serif");
      ...
      $card.find("[data-ecf-font-pairing-apply]").attr("data-ecf-font-pairing-slug", slug).attr("data-ecf-font-pairing-heading", headingFamily).attr("data-ecf-font-pairing-body", bodyFamily);
    }
    ...
    function buildTypePreviewItems(config) {
      ...
      return $.map(config.steps, function(step, i) {
        var exp = i - baseIndex;
        var maxSize = Math.round(config.maxBase * Math.pow(config.maxRatio, exp) * 1e3) / 1e3;
        var minSize = Math.round(config.minBase * Math.pow(config.minRatio, exp) * 1e3) / 1e3;
        if (minSize > maxSize) {
          var _s = minSize;
          minSize = maxSize;
          maxSize = _s;
        ...
        if (config.fluid && config.maxVw > config.minVw) {
          ...
          cssValue = "clamp(" + formatPreviewNumber(pxToRem(minSize)) + "rem, calc(" + formatPreviewNumber(slopeVw) + "vw " + (interceptRem >= 0 ? "+ " : "- ") + formatPreviewNumber(Math.abs(interceptRem)) + "rem), " + formatPreviewNumber(pxToRem(maxSize)) + "rem)";
        } else {
          minSize = maxSize;
        }
        return {
    ...
    function getTypePreviewText(stepOrToken, $scope) {
      var normalized = String(stepOrToken || "").toLowerCase().replace(/^--ecf-text-/, "").trim();
    ...
    function renderTypePreview() {
      ...
      $.each(items, function(_, item) {
        ...
        html += '<div class="ecf-type-row' + selectedClass + '" data-ecf-step="' + item.step + '" data-ecf-step-row tabindex="0" role="button" aria-pressed="' + (item.step === activeStep ? "true" : "false") + '" style="--ecf-preview-size:' + sizeForView(item) + ';"><div class="ecf-type-row__token"><div class="ecf-type-row__token-line"><span class="ecf-type-row__token-label">' + item.token + '</span><button type="button" class="ecf-clamp-toggle" data-ecf-clamp-toggle="' + escapeHtml(i18n.copy) + '"><span class="dashicons dashicons-editor-code"></span></button><span class="ecf-copy-pill" data-copy="' + item.token + '">' + i18n.copy + '</span></div><button type="button" class="ecf-clamp-popover" data-copy="' + escapeHtml(item.cssValue) + '">' + escapeHtml(item.cssValue) + '</button></div><div class="ecf-type-row__meta"><div><span><i class="dashicons dashicons-smartphone"></i>' + labelMin + '</span><div class="ecf-clamp-metric"><strong>' + item.minPx + 'px</strong></div></div><div><span><i class="dashicons dashicons-desktop"></i>' + labelMax + '</span><div class="ecf-clamp-metric"><strong>' + item.maxPx + 'px</strong></div></div></div><div class="ecf-type-row__sample"><div class="ecf-type-row__sample-line"><strong style="font-size:' + item.minPx + 'px;">' + escapeHtml(previewText) + '</strong><span><i class="dashicons dashicons-smartphone"></i>' + labelMin + '</span></div><div class="ecf-type-row__sample-line ecf-type-row__sample-line--max"><strong style="font-size:' + item.maxPx + 'px;">' + escapeHtml(previewText) + '</strong><span><i class="dashicons dashicons-desktop"></i>' + labelMax + "</span></div></div></div>";
      });
    ...
    function buildRadiusCssValue(minPx, maxPx, rootBasePx) {
      ...
      var slopeVw = Math.round(slope * 100 * 100) / 100;
      return "clamp(" + formatPreviewNumber(minPx / rootBasePx) + "rem, calc(" + formatPreviewNumber(slopeVw) + "vw " + (interceptRem >= 0 ? "+ " : "- ") + formatPreviewNumber(Math.abs(interceptRem)) + "rem), " + formatPreviewNumber(maxPx / rootBasePx) + "rem)";
    }
    function renderRootFontImpact() {
      ...
      var typeItem = typeItems.find(function(item) {
        return item.step === typeStep;
      }) || typeItems[0];
      var spacingItem = spacingItems.find(function(item) {
        return item.step === spacingStep;
      }) || spacingItems[0];
    ...
    function buildShadowPreviewItems() {
      return $('.ecf-table[data-group="shadows"] .ecf-row').map(function(index) {
        ...
        var slug = name.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "") || "shadow-" + index;
        return {
          index,
          name,
          slug,
    ...
    function renderShadowPreview() {
      ...
      $.each(items, function(_, item) {
        ...
        html += '<div class="ecf-shadow-row' + selectedClass + '" data-ecf-shadow-step="' + item.slug + '" data-ecf-shadow-index="' + item.index + '"><div class="ecf-shadow-row__class"><code>' + escapeHtml(item.className) + '</code></div><div class="ecf-shadow-row__token">' + escapeHtml(item.token) + '</div><div class="ecf-shadow-row__value"><input type="text" class="ecf-shadow-row__value-input" data-ecf-shadow-inline-value data-ecf-shadow-index="' + item.index + '" value="' + escapeHtml(item.value) + '" spellcheck="false" autocomplete="off"></div><div class="ecf-shadow-row__sample ecf-shadow-preview-bg"><div class="ecf-shadow-row__mini" style="box-shadow:' + escapeHtml(previewShadowValue) + ';"></div></div></div>';
      });
      var activeItem = items.find(function(item) {
        return item.slug === activeShadow;
      }) || items[0];
      ...
      if (typeof syncShadowSubnavActive === "function") {
        syncShadowSubnavActive();
      }
    }
    function enterTokenFocus(type, slug) {
      var $panel = $('.ecf-panel[data-panel="tokens"]');
      var $card = type === "color" ? $panel.find('[data-ecf-layout-item="tokens-colors"]') : $panel.find('[data-ecf-layout-item="tokens-radius"]');
      ...
      if (type === "color") {
        var $detail = $focused.find(".ecf-color-detail").first();
        if ($detail.length && $detail.prop("hidden")) {
          $focused.find(".ecf-color-detail-toggle").first().trigger("click");
        }
      }
      $panel.find(".ecf-card.ecf-panel-shell").attr("data-ecf-token-focus", type);
      if (!$card.find(".ecf-token-focus-bar").length) {
        var tokenName = type === "color" ? "--ecf-color-" + slug : "--ecf-radius-" + slug;
        var $bar = $('<div class="ecf-token-focus-bar"><button type="button" class="ecf-token-focus-back" data-ecf-focus-back><span class="dashicons dashicons-arrow-left-alt2"></span>' + (type === "color" ? "Alle Farben" : "Alle Radius") + '</button><span class="ecf-token-focus-label">' + escapeHtml(tokenName) + "</span></div>");
        $card.find("h2").first().after($bar);
      } else {
        var tokenName = type === "color" ? "--ecf-color-" + slug : "--ecf-radius-" + slug;
        $card.find(".ecf-token-focus-label").text(tokenName);
      }
      $('[data-ecf-subnav="tokens"] .ecf-nav-subnav__item').removeClass("is-active");
      $('[data-ecf-subnav="tokens"] [data-ecf-subnav-token="' + type + '"][data-ecf-subnav-slug="' + slug + '"]').addClass("is-active");
      scheduleMasonryLayouts();
    }
    function exitTokenFocus() {
      var $panel = $('.ecf-panel[data-panel="tokens"]');
      $panel.find(".ecf-card.ecf-panel-shell").removeAttr("data-ecf-token-focus");
      $panel.find(".ecf-row--color, .ecf-row--minmax").removeClass("ecf-row--focused");
      $('[data-ecf-subnav="tokens"] .ecf-nav-subnav__item').removeClass("is-active");
      scheduleMasonryLayouts();
    }
    $(document).on("click", "[data-ecf-focus-back]", function() {
      exitTokenFocus();
    });
    function buildShadowSubnav() {
      var $subnav = $('[data-ecf-subnav="shadows"]');
      ...
      items.forEach(function(item) {
        var isActive = item.slug === activeShadow;
        html += '<button type="button" class="ecf-nav-subnav__item' + (isActive ? " is-active" : "") + '" data-ecf-subnav-shadow="' + item.slug + '"><span class="ecf-nav-subnav__dot"></span><span class="ecf-nav-subnav__label">' + item.name + '</span><span class="ecf-nav-subnav__token">--ecf-shadow-' + item.slug + "</span></button>";
      });
      $subnav.html(html);
    }
    ...
    function enterShadowFocus(slug) {
      ...
      var $editorRows = $("[data-ecf-shadow-edit-row]");
      $editorRows.removeClass("ecf-shadow-focused-editor");
      var $matchedEditor = $editorRows.filter(function() {
        var $nameInput = $(this).find("[data-ecf-shadow-name-input]");
        var name = $nameInput.val() || "";
        return name.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "") === slug;
      }).first();
      $matchedEditor.addClass("ecf-shadow-focused-editor");
      var $bar = $preview.find(".ecf-shadow-focus-bar");
      if (!$bar.length) {
        $bar = $('<div class="ecf-shadow-focus-bar"><button type="button" class="ecf-token-focus-back" data-ecf-shadow-focus-back><span class="dashicons dashicons-arrow-left-alt2"></span>Alle Schatten</button><span class="ecf-token-focus-label">--ecf-shadow-' + escapeHtml(slug) + "</span></div>");
        $preview.find(".ecf-shadow-preview-header").after($bar);
      } else {
        $bar.prop("hidden", false);
        $bar.find(".ecf-token-focus-label").text("--ecf-shadow-" + slug);
      }
      if ($matchedEditor.length) {
        $matchedEditor[0].scrollIntoView({ behavior: "smooth", block: "center" });
      }
      syncShadowSubnavActive();
    }
    function exitShadowFocus() {
      ...
      $('[data-ecf-subnav="shadows"] .ecf-nav-subnav__item').removeClass("is-active");
    }
    $(document).on("click", "[data-ecf-shadow-focus-back]", function() {
      exitShadowFocus();
    });
    ...
    function enterSpacingFocus(step) {
      ...
      var $bar = $preview.find(".ecf-spacing-focus-bar");
      if (!$bar.length) {
        $bar = $('<div class="ecf-spacing-focus-bar"><button type="button" class="ecf-token-focus-back" data-ecf-spacing-focus-back><span class="dashicons dashicons-arrow-left-alt2"></span>Alle Schritte</button><span class="ecf-token-focus-label">--ecf-space-' + escapeHtml(step) + "</span></div>");
        $preview.find("[data-ecf-spacing-preview-list]").before($bar);
      } else {
        $bar.find(".ecf-token-focus-label").text("--ecf-space-" + step);
        $bar.prop("hidden", false);
      }
      $('[data-ecf-subnav="spacing"] .ecf-nav-subnav__item').removeClass("is-active");
    ...
    $(document).on("click", "[data-ecf-spacing-focus-back]", function() {
      exitSpacingFocus();
    });
    function buildTokensSubnav() {
      ...
      if ($colorRows.length) {
        ...
        html += '<button type="button" class="ecf-nav-subnav__section ecf-nav-subnav__section--toggle' + (colorsOpen ? " is-open" : "") + '" data-ecf-subnav-group="colors">' + colorLabel + "</button>";
        ...
        $colorRows.each(function() {
          ...
          html += '<button type="button" class="ecf-nav-subnav__item" data-ecf-subnav-token="color" data-ecf-subnav-slug="' + slug + '"><span class="ecf-nav-subnav__swatch" style="background:' + escapeHtml(hex) + ';"></span><span class="ecf-nav-subnav__label">' + escapeHtml(name) + "</span></button>";
        });
        html += "</div>";
      }
      var $radiusRows = $('[data-group="radius"] .ecf-row--minmax');
      if ($radiusRows.length) {
        html += '<button type="button" class="ecf-nav-subnav__section ecf-nav-subnav__section--toggle' + (radiusOpen ? " is-open" : "") + '" data-ecf-subnav-group="radius">Radius</button>';
        html += '<div class="ecf-nav-subnav__group' + (radiusOpen ? " is-open" : "") + '" data-ecf-subnav-group-items="radius">';
        $radiusRows.each(function() {
          var name = $.trim($(this).find('[data-ecf-slug-field="token"]').val() || "");
          if (!name) return;
          var slug = name.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "");
          html += '<button type="button" class="ecf-nav-subnav__item" data-ecf-subnav-token="radius" data-ecf-subnav-slug="' + slug + '"><span class="ecf-nav-subnav__dot"></span><span class="ecf-nav-subnav__label">' + escapeHtml(name) + '</span><span class="ecf-nav-subnav__token">--ecf-radius-' + escapeHtml(slug) + "</span></button>";
        });
        html += "</div>";
      }
      $subnav.html(html);
    }
    $(document).on("click", ".ecf-nav-subnav__section--toggle", function() {
      var group = String($(this).data("ecf-subnav-group") || "");
      var isOpen = $(this).hasClass("is-open");
      $(this).toggleClass("is-open", !isOpen);
      $('[data-ecf-subnav-group-items="' + group + '"]').toggleClass("is-open", !isOpen);
    });
    $(document).on("click", "[data-ecf-subnav-token]", function() {
      var type = String($(this).data("ecf-subnav-token") || "");
      var slug = String($(this).data("ecf-subnav-slug") || "");
      if (!slug) return;
      var $shell = $('.ecf-panel[data-panel="tokens"] .ecf-card.ecf-panel-shell');
      if ($shell.attr("data-ecf-token-focus") === type && $(this).hasClass("is-active")) {
        exitTokenFocus();
        return;
      }
    ...
    function updateLayrixVariableCount(settings, meta) {
      var count = meta && meta.layrix_variable_count != null ? parseInt(meta.layrix_variable_count, 10) : countLayrixVariablesFromSettings(settings || buildSettingsPayloadFromForm());
    ...
    function getCurrentPanel() {
      return $(".ecf-nav-item.is-active").attr("data-panel") || "";
    }
    ...
    function updateSaveButtonVisibility() {
      ...
      if (isNoSavePanel || !isDirty) {
        ...
        if ($save.prop("hidden")) {
          $save.prop("hidden", false).addClass("ecf-save-entrance");
          setTimeout(function() {
            $save.removeClass("ecf-save-entrance");
          }, 400);
        }
        $topbar.addClass("is-dirty");
      }
    }
    ...
    function updateSystemInfoCards(meta, settings) {
      ...
      if (debug) {
        ...
        $("[data-ecf-debug-limits]").text(
          String(i18n.limit_summary || "").replace("%1$s", String(debug.classes_limit)).replace("%2$s", String(debug.variables_limit))
        );
        $("[data-ecf-debug-limit-sources]").text(
          String(i18n.source_limits || "").replace("%1$s", debug.classes_limit_source || "").replace("%2$s", debug.variables_limit_source || "")
        );
    ...
    function applyMasonryLayoutToGroup($group) {
      ...
      var columns = declaredColumns && declaredColumns > 0 ? declaredColumns : countGridTracks(computedGroupStyle.gridTemplateColumns);
    ...
    function hasCustomLayoutState() {
      ...
      return stableStringify(currentOrders) !== stableStringify(normalizeLayoutOrders(defaultLayoutOrders)) || stableStringify(currentColumns) !== stableStringify(normalizeLayoutColumns(defaultLayoutColumns));
    }
    ...
    function switchPanel(panel) {
      ...
      $('[data-ecf-subnav="shadows"]').prop("hidden", panel !== "shadows");
      if (panel === "tokens") {
        buildTokensSubnav();
      }
      $('[data-ecf-subnav="tokens"]').prop("hidden", panel !== "tokens");
      if (panel === "typography") {
        buildTypographySubnav();
      }
      $('[data-ecf-subnav="typography"]').prop("hidden", panel !== "typography");
      if (panel === "spacing") {
        buildSpacingSubnav();
      }
      $('[data-ecf-subnav="spacing"]').prop("hidden", panel !== "spacing");
      if (panel === "components") {
        buildComponentsSubnav();
      }
      $('[data-ecf-subnav="components"]').prop("hidden", panel !== "components");
      $('[data-ecf-subnav="starthilfe"]').prop("hidden", panel !== "starthilfe");
      refreshSortableLayoutGroups();
      scheduleMasonryLayouts();
    }
    ...
    function switchWebsiteTab(tab) {
      ...
      $("[data-ecf-website-tab]").removeClass("is-active").attr("aria-pressed", "false").filter('[data-ecf-website-tab="' + activeTab + '"]').addClass("is-active").attr("aria-pressed", "true");
      $("[data-ecf-website-section]").removeClass("is-active").prop("hidden", true).filter('[data-ecf-website-section="' + activeTab + '"]').addClass("is-active").prop("hidden", false);
      try {
    ...
    function switchGeneralTab(tab) {
      ...
      $("[data-ecf-general-tab]").removeClass("is-active").filter('[data-ecf-general-tab="' + activeTab + '"]').addClass("is-active");
      $("[data-ecf-general-section]").removeClass("is-active").prop("hidden", true).filter('[data-ecf-general-section="' + activeTab + '"]').addClass("is-active").prop("hidden", false);
      try {
      ...
      refreshSortableLayoutGroups();
      scheduleMasonryLayouts();
      syncComponentsSubnavActive();
    }
    function switchTypographyTab(tab) {
      var activeTab = normalizeTypographyTab(tab);
      $("[data-ecf-typography-tab]").removeClass("is-active").attr("aria-pressed", "false").filter('[data-ecf-typography-tab="' + activeTab + '"]').addClass("is-active").attr("aria-pressed", "true");
      $("[data-ecf-typography-section]").removeClass("is-active").prop("hidden", true).filter('[data-ecf-typography-section="' + activeTab + '"]').addClass("is-active").prop("hidden", false);
      try {
    ...
    function buildTypographySubnav() {
      var $subnav = $('[data-ecf-subnav="typography"]');
      if (!$subnav.length) return;
      var activeTab = normalizeTypographyTab(
        $("[data-ecf-typography-tab].is-active").data("ecf-typography-tab") || "fonts"
      );
      var items = [
        { tab: "fonts", icon: "dashicons-editor-textcolor", label: "Schriftpaare" },
        { tab: "scale", icon: "dashicons-visibility", label: "Vorschau" }
      ];
      var html = "";
      items.forEach(function(item) {
        html += '<button type="button" class="ecf-nav-subnav__item' + (item.tab === activeTab ? " is-active" : "") + '" data-ecf-subnav-typography="' + item.tab + '"><span class="ecf-nav-subnav__dot"></span><span class="ecf-nav-subnav__label">' + item.label + "</span></button>";
      });
      $subnav.html(html);
    }
    function syncTypographySubnavActive(activeTab) {
      $('[data-ecf-subnav="typography"] .ecf-nav-subnav__item').each(function() {
        $(this).toggleClass("is-active", $(this).data("ecf-subnav-typography") === activeTab);
      });
    }
    function buildSpacingSubnav() {
      var $subnav = $('[data-ecf-subnav="spacing"]');
      if (!$subnav.length) return;
      var steps = getSpacingSteps ? getSpacingSteps() : [];
      if (!steps.length) {
        $subnav.html("");
        return;
      }
      ...
      steps.forEach(function(step) {
        html += '<button type="button" class="ecf-nav-subnav__item" data-ecf-subnav-spacing="' + escapeHtml(step) + '"><span class="ecf-nav-subnav__dot"></span><span class="ecf-nav-subnav__label">' + escapeHtml(step) + '</span><span class="ecf-nav-subnav__token">--ecf-space-' + escapeHtml(step) + "</span></button>";
      });
      $subnav.html(html);
    }
    $(document).on("click", "[data-ecf-subnav-spacing]", function() {
      ...
      enterSpacingFocus(step);
    });
    $(document).on("click", "[data-ecf-starthilfe-jump]", function() {
      var section = String($(this).data("ecf-starthilfe-jump") || "");
      if (!section) return;
      $('[data-ecf-subnav="starthilfe"] .ecf-nav-subnav__item').removeClass("is-active");
      $(this).addClass("is-active");
      var $target = $("#starthilfe-" + section);
      if (!$target.length) return;
      var $scroller = $(".ecf-main");
      var scrollTop = $scroller.scrollTop();
      var containerTop = $scroller.offset().top;
      var targetTop = $target.offset().top;
      $scroller.animate({ scrollTop: scrollTop + targetTop - containerTop - 16 }, 250);
    });
    var componentsSubnavItems = [
      { tab: "website", label: "Website" },
      { tab: "interface", label: "Interface" },
      { tab: "system", label: "System" },
      { tab: "favorites", label: "Favoriten" }
    ];
    function buildComponentsSubnav() {
      var $subnav = $('[data-ecf-subnav="components"]');
      if (!$subnav.length) return;
      var activeTab = String($("[data-ecf-general-tab].is-active").data("ecf-general-tab") || "website");
      var html = "";
      componentsSubnavItems.forEach(function(item) {
        html += '<button type="button" class="ecf-nav-subnav__item' + (item.tab === activeTab ? " is-active" : "") + '" data-ecf-subnav-components="' + item.tab + '"><span class="ecf-nav-subnav__dot"></span><span class="ecf-nav-subnav__label">' + item.label + "</span></button>";
      });
      $subnav.html(html);
    }
    function syncComponentsSubnavActive() {
      var activeTab = String($("[data-ecf-general-tab].is-active").data("ecf-general-tab") || "website");
      $('[data-ecf-subnav="components"] .ecf-nav-subnav__item').each(function() {
        $(this).toggleClass("is-active", $(this).data("ecf-subnav-components") === activeTab);
      });
    }
    $(document).on("click", "[data-ecf-subnav-components]", function() {
      var tab = String($(this).data("ecf-subnav-components") || "");
      if (!tab) return;
      if (!$('.ecf-panel[data-panel="components"]').hasClass("is-active")) {
        switchPanel("components");
      }
      switchGeneralTab(tab);
      syncComponentsSubnavActive();
    });
    $(document).on("click", "[data-ecf-subnav-typography]", function() {
      var tab = String($(this).data("ecf-subnav-typography") || "");
      if (!tab) return;
      if (!$('.ecf-panel[data-panel="typography"]').hasClass("is-active")) {
        switchPanel("typography");
      }
      switchTypographyTab(tab);
    });
    ...
    updateBaseBodyTextSizeWarning();
    syncTypographyFontCardSummaries();
    initUiWidgets($, { restUrl, restNonce, buildVariableSyncHash, buildSettingsPayloadFromForm, switchPanel });
    ...
    function renderVariableSizeMetrics(minPx, maxPx, visualHtml, modifierClass) {
      ...
      return '<span class="ecf-var-value ecf-var-value--size ' + (modifierClass || "") + '"><span class="ecf-var-space-metric"><small>' + escapeHtml(i18n.preview_min || "") + "</small><strong>" + escapeHtml(formatNumber(minPx, 2)) + 'px</strong></span><span class="ecf-var-space-visual" aria-hidden="true">' + (visualHtml || '<span class="ecf-var-space-bar"><span class="ecf-var-space-fill" style="width:' + minWidth + '%;height:6px;"></span></span><span class="ecf-var-space-bar"><span class="ecf-var-space-fill" style="width:' + maxWidth + '%;height:10px;"></span></span>') + '</span><span class="ecf-var-space-metric"><small>' + escapeHtml(i18n.preview_max || "") + "</small><strong>" + escapeHtml(formatNumber(maxPx, 2)) + "px</strong></span></span>";
    }
    function renderVariableValue(item) {
      ...
      if (kind === "radius" || kind === "typography") {
        ...
        if (sizeData) {
          var visualHtml = "";
          if (kind === "radius") {
            visualHtml = '<span class="ecf-var-radius-visual-row"><span class="ecf-var-radius-box" style="border-radius:' + escapeHtml(formatNumber(sizeData.minPx, 2)) + 'px;"></span><span class="ecf-var-radius-box ecf-var-radius-box--max" style="border-radius:' + escapeHtml(formatNumber(sizeData.maxPx, 2)) + 'px;"></span></span>';
          } else {
            visualHtml = '<span class="ecf-var-type-visual-row"><span class="ecf-var-type-sample" style="font-size:' + escapeHtml(formatNumber(sizeData.minPx, 2)) + 'px;">' + escapeHtml(i18n.search_preview_text_sample || "") + '</span><span class="ecf-var-type-sample ecf-var-type-sample--max" style="font-size:' + escapeHtml(formatNumber(sizeData.maxPx, 2)) + 'px;">' + escapeHtml(i18n.search_preview_text_sample || "") + "</span></span>";
          }
    ...
    function buildVarTable(group, items) {
      var nameColumnLabel = isClassGroup(group) ? i18n.col_name || "" : i18n.col_variable_name || i18n.col_name || "";
    ...
    function renderVarList(group, items) {
      ...
      $.each(order, function(_, key) {
        ...
        tabs += '<button type="button" class="ecf-var-tab' + (key === varTabs[group] ? " is-active" : "") + '" data-group="' + group + '" data-var-tab="' + key + '">' + typeTabLabel(key) + '<span class="ecf-var-tab__count">' + buckets[key].length + "</span></button>";
      });
      tabs += "</div>";
    ...
    function updateSelectAllState(group) {
      ...
      $btn.prop("disabled", false).toggleClass("is-active", allChecked).find("span:last-child").text(allChecked ? i18n.deselect_all : i18n.select_all);
      $deleteBtn.prop("disabled", checkedCount === 0).toggleClass("is-active", checkedCount > 0);
    }
    ...
    function updateClassSummary(ecfItems, foreignItems) {
      ...
      var labels = [].concat((ecfItems || []).map(function(item) {
        return String(item.label || "").toLowerCase();
      })).concat((foreignItems || []).map(function(item) {
        return String(item.label || "").toLowerCase();
      }));
    ...
    function applyClassUsageStatus(total) {
      $("[data-ecf-class-usage-card]").each(function() {
        ...
        $card.removeClass("ecf-class-limit-card--neutral ecf-class-limit-card--success ecf-class-limit-card--warning ecf-class-limit-card--danger").addClass("ecf-class-limit-card--" + status);
      });
    }
    function getStarterExistingLabels() {
      var raw = $("[data-ecf-starter-classes]").attr("data-ecf-existing-labels") || "[]";
      try {
        return JSON.parse(raw).map(function(label) {
    ...
    function renderActiveClassGroup(type, names) {
      ...
      if (type === "existing-foreign") {
        ...
        if (legacyNames.length) {
          html += '<div class="ecf-active-class-subgroup"><h4 class="ecf-active-class-subgroup__title">' + escapeHtml(i18n.existing_foreign_legacy_heading || "") + '</h4><p class="ecf-active-class-subgroup__hint">' + escapeHtml(i18n.existing_foreign_legacy_hint || "") + '</p><ul class="ecf-active-class-plain-list">' + legacyNames.map(function(name) {
            return '<li class="ecf-active-class-item ecf-active-class-item--legacy">' + escapeHtml(name) + "</li>";
          }).join("") + "</ul></div>";
        }
        if (manualNames.length) {
          html += '<div class="ecf-active-class-subgroup"><h4 class="ecf-active-class-subgroup__title">' + escapeHtml(i18n.existing_foreign_manual_heading || "") + '</h4><p class="ecf-active-class-subgroup__hint">' + escapeHtml(i18n.existing_foreign_manual_hint || "") + '</p><ul class="ecf-active-class-plain-list">' + manualNames.map(function(name) {
    ...
    function updateActiveClassTierVisibility(activeTier) {
      ...
      $("[data-ecf-active-class-group]").each(function() {
        ...
        var hasItems = $group.find(".ecf-active-class-item").length > 0 || $list.find(".ecf-active-class-empty").length > 0 || $.trim($list.text()).length > 0;
    ...
    function updateActiveClassSummary(starterNames, utilityNames, syncPayloadNames) {
      ...
      $("[data-ecf-active-class-hint]").text(
        helperNames.length ? i18n.active_class_hint_helper || "" : i18n.active_class_hint_default || ""
      );
    ...
    function updateStarterClassesState() {
      ...
      var pendingNew = selectedNames.filter(function(name) {
      ...
      $("[data-ecf-starter-status]").removeClass("ecf-class-limit-card--neutral ecf-class-limit-card--success ecf-class-limit-card--warning ecf-class-limit-card--danger").addClass("ecf-class-limit-card--" + status);
    ...
    function applyClassTierFilter(tier) {
      ...
      $("[data-ecf-class-tier]").removeClass("is-active").filter('[data-ecf-class-tier="' + activeTier + '"]').addClass("is-active");
    ...
    function updateClassLibrarySelectAllState() {
      ...
      $button.prop("disabled", !$checks.length).toggleClass("is-active", allChecked).find("[data-ecf-class-select-all-label]").text(allChecked ? i18n.deselect_all || "" : i18n.select_all || "");
      $("[data-ecf-class-select-all-icon]").toggleClass("is-active", allChecked || hasSelection);
    }
    function normalizeBemSegment(value) {
      return String(value || "").toLowerCase().trim().replace(/^ecf-/, "").replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "").replace(/-{2,}/g, "-");
    }
    function parseBemCsv(value) {
      return String(value || "").split(",").map(function(part) {
        return normalizeBemSegment(part);
      }).filter(Boolean);
    }
    ...
    function renderBemGeneratorOptions(type, values) {
      return (values || []).map(function(value) {
        ...
        return '<label class="ecf-bem-generator__option"><input type="checkbox" data-ecf-bem-' + type + '-check value="' + escapeHtml(label) + '"><span>' + escapeHtml(label) + "</span></label>";
      }).join("");
    }
    ...
    function getBemGeneratedClasses() {
      ...
      var generated = [block];
      var elements = [].concat($root.find("[data-ecf-bem-element-check]:checked").map(function() {
        return $(this).val();
      }).get()).concat(parseBemCsv($root.find("[data-ecf-bem-extra-elements]").val()));
      var modifiers = [].concat($root.find("[data-ecf-bem-modifier-check]:checked").map(function() {
        return $(this).val();
      }).get()).concat(parseBemCsv($root.find("[data-ecf-bem-extra-modifiers]").val()));
    ...
    function renderBemGeneratorPreview() {
      ...
      var emptyText = isBemCustomPreset() ? i18n.bem_preview_empty_custom || "" : i18n.bem_preview_empty_preset || "";
      var html = classes.length ? classes.map(function(className) {
        return "<code>" + escapeHtml(className) + "</code>";
      }).join("") : '<span class="ecf-muted-copy">' + escapeHtml(emptyText) + "</span>";
    ...
    function appendCustomStarterRow(config) {
      ...
      template = template.replace("__ENABLED__", "ecf_framework_v50[starter_classes][custom][" + index + "][enabled]").replace("__NAME__", "ecf_framework_v50[starter_classes][custom][" + index + "][name]").replace("__CATEGORY__", "ecf_framework_v50[starter_classes][custom][" + index + "][category]");
      var $row = $(template);
    ...
    function addBemGeneratedClasses() {
      ...
      $.each(classes, function(_, className) {
        ...
        var $row = appendCustomStarterRow({ name: normalized, category, enabled: true });
        if ($row && $row.length) {
          $row.addClass("is-new");
          addedRows.push($row);
        }
      ...
      $root.find("[data-ecf-bem-feedback]").text(
        added ? formatTemplate(added === 1 ? i18n.bem_feedback_added_one || "" : i18n.bem_feedback_added_many || "", added) : i18n.bem_feedback_exists || ""
      );
      updateStarterClassesState();
      if (!added) {
        refreshStarterClassVisibility();
    ...
    function showClassLibrarySection(section) {
      ...
      $("[data-ecf-library-section]").attr("hidden", true).filter('[data-ecf-library-section="' + activeSection + '"]').removeAttr("hidden");
    }
    ...
    function refreshStarterClassVisibility() {
      ...
      $("[data-ecf-starter-item]").each(function() {
        ...
        var matchesTier = activeTier === "all" || activeTier === "basic" || activeTier === "extras" ? activeTier === "all" ? true : activeTier === "extras" ? itemTier === "advanced" : itemTier === activeTier : activeTier === "custom" ? false : true;
    ...
    function applyUtilityClassFilter(category) {
      var activeCategory = category || "all";
      var $activeTab = $("[data-ecf-utility-tab]").removeClass("is-active").filter('[data-ecf-utility-tab="' + activeCategory + '"]').addClass("is-active");
    ...
    function renderGlobalSearchResults(query) {
      ...
      $.each(matches.slice(0, 24), function(_, item) {
        ...
        html += '<div class="ecf-global-search__item"><button type="button" class="ecf-global-search__main" data-ecf-search-result data-group="' + item.group + '" data-id="' + item.id + '" data-tab-key="' + item.tabKey + '"><div class="ecf-global-search__identity"><span class="ecf-global-search__meta">' + escapeHtml(groupLabel(item.group)) + '</span><strong class="ecf-global-search__label">' + originBadge(item.group) + "<span>" + escapeHtml(item.label) + '</span></strong></div><span class="ecf-global-search__type">' + escapeHtml(typeLabel(item.type)) + "</span>" + renderSearchPreview(item) + '</button><div class="ecf-global-search__actions">' + actionsHtml + "</div></div>";
      });
      html += "</div>";
    ...
    function normalizeSearchEditLabel(rawValue) {
      return String(rawValue || "").trim().toLowerCase().replace(/[^a-z0-9\-_ ]+/g, "").replace(/\s+/g, "-").replace(/-+/g, "-").replace(/^-+|-+$/g, "");
    }
    ...
    function updateSearchEditClampInfo(clampData) {
      ...
      if (type === "global-size-variable" && clampData) {
        $tech.html("<strong>" + escapeHtml(i18n.clamp_label || "") + "</strong><br><code>" + escapeHtml(clampData.raw) + "</code>").prop("hidden", false);
    ...
    function inferSearchEditType(item) {
      ...
      var looksLikeSize = /^-?\d+(?:\.\d+)?(?:px|rem|em|ch|%|vw|vh)$/i.test(value) || /^(?:clamp|min|max|calc)\(/i.test(value);
      var isTypographyLike = label.indexOf("text-") === 0 || label.indexOf("font-") === 0 || label.indexOf("size") !== -1 || label.indexOf("typography") !== -1;
    ...
    $(document).on("click", ".ecf-delete-selected", function() {
      var group = $(this).data("group");
      var isClassGroup2 = String(group).indexOf("classes") !== -1;
      var ids = [];
      var selectedItems = [];
      ...
      if (isClassGroup2) {
        ...
        var usedItems = selectedItems.filter(function(item) {
          return !!item.in_use;
        });
        var unusedItems = selectedItems.filter(function(item) {
          return !item.in_use;
        });
        if (usedItems.length) {
          openClassDeleteModal({
            ...
            unusedItems,
            allIds: ids.slice(),
            unusedIds: unusedItems.map(function(item) {
      ...
      $.post(ecfAdmin.ajaxurl, {
        action: isClassGroup2 ? "ecf_delete_classes" : "ecf_delete_variables",
        nonce: ecfAdmin.nonce,
        ids
      }, function(res) {
    ...
    $(document).on("click", "[data-ecf-search-edit]", function(e) {
      ...
      if (!item) return;
      if (!canEditSearchItem($.extend({}, item, { group }))) {
        alert(isClassGroup(group) ? i18n.search_edit_class : i18n.search_edit_generated);
        return;
      }
      openSearchEditModal($.extend({}, item, { group }));
    });
    ...
    $(document).on("click", "[data-ecf-search-edit-save]", function() {
      ...
      if (type === "global-color-variable" && $("[data-ecf-search-edit-color]").val()) {
        ...
        if (clampData) {
          ...
          value = "clamp(" + pxToSizeToken(minPx, clampData.minToken.unit, rootBasePx) + ", " + clampData.middle + ", " + pxToSizeToken(maxPx, clampData.maxToken.unit, rootBasePx) + ")";
        } else {
    ...
    function buildSpacingItems(steps, cfg) {
      ...
      return $.map(steps, function(step, i) {
        ...
        if (exp === 0) {
          ...
          maxSize = cfg.maxBase * Math.pow(cfg.maxRatio, exp);
          minSize = cfg.minBase * Math.pow(cfg.minRatio, exp);
        } else {
          maxSize = cfg.maxBase / Math.pow(cfg.maxRatio, Math.abs(exp));
          minSize = cfg.minBase / Math.pow(cfg.minRatio, Math.abs(exp));
        }
        maxSize = Math.round(maxSize * 1e3) / 1e3;
        minSize = Math.round(minSize * 1e3) / 1e3;
        if (minSize > maxSize) {
          var swap = minSize;
          minSize = maxSize;
          maxSize = swap;
        ...
        return { step, token: "--ecf-" + cfg.prefix + "-" + step, min: formatPreviewNumber(pxToRem(minSize)), max: formatPreviewNumber(pxToRem(maxSize)), minPx: formatPreviewNumber(minSize), maxPx: formatPreviewNumber(maxSize), cssValue, isBase: i === baseIdx };
      });
    }
    function renderSpacingPreview() {
      ...
      $.each(items, function(_, item) {
        ...
        html += '<div class="ecf-space-row' + (item.isBase ? " is-base" : "") + '" data-ecf-space-step="' + item.step + '"><div class="ecf-space-row__token"><span class="ecf-space-row__token-text ecf-spacing-token-name">' + item.token + '</span><span class="ecf-copy-pill" data-copy="' + item.token + '">' + i18n.copy + '</span></div><div class="ecf-space-row__meta"><div class="ecf-space-row__metric"><div class="ecf-space-row__metric-meta"><span><i class="dashicons dashicons-smartphone"></i>' + labelMin + '</span><div class="ecf-clamp-metric"><strong>' + formatPreviewNumber(minValue) + 'px</strong><button type="button" class="ecf-clamp-toggle" data-ecf-clamp-toggle="' + escapeHtml(i18n.copy) + '"><span class="dashicons dashicons-editor-code"></span></button></div><button type="button" class="ecf-clamp-popover" data-copy="' + escapeHtml(item.cssValue) + '">' + escapeHtml(item.cssValue) + '</button></div><div class="ecf-space-row__bar"><div class="ecf-space-row__bar-fill" style="width:' + formatPreviewNumber(minBarWidth) + "px;height:" + minBarH + 'px;"></div></div></div><div class="ecf-space-row__metric"><div class="ecf-space-row__metric-meta"><span><i class="dashicons dashicons-desktop"></i>' + labelMax + '</span><div class="ecf-clamp-metric"><strong>' + formatPreviewNumber(maxValue) + 'px</strong><button type="button" class="ecf-clamp-toggle" data-ecf-clamp-toggle="' + escapeHtml(i18n.copy) + '"><span class="dashicons dashicons-editor-code"></span></button></div><button type="button" class="ecf-clamp-popover" data-copy="' + escapeHtml(item.cssValue) + '">' + escapeHtml(item.cssValue) + '</button></div><div class="ecf-space-row__bar"><div class="ecf-space-row__bar-fill" style="width:' + formatPreviewNumber(maxBarWidth) + "px;height:" + maxBarH + 'px;"></div></div></div></div></div>';
      });
    ...
    function applySteps(steps) {
      ...
      try {
        ...
        console.error("ECF renderTypePreview error:", err);
      }
      scheduleSettingsAutosave({ delay: 250 });
    }
