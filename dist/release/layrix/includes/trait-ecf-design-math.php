...
trait ECF_Framework_Design_Math_Trait {
    ...
    private function generated_color_variants($color, $options = []) {
        $parsed = $this->parse_css_color($color);
        if (!$parsed) {
            return [];
        }
        ...
        $base = [
            ...
            'g' => (int) $parsed['g'],
            'b' => (int) $parsed['b'],
            'a' => $this->normalize_alpha($parsed['a'] ?? 1),
        ];
        $variants = [];
        $variant_groups = [
            'shade' => [
                'enabled' => !empty($options['generate_shades']),
                'count' => $options['shade_count'] ?? 6,
                'target' => ['r' => 0, 'g' => 0, 'b' => 0],
            ],
            'tint' => [
                'enabled' => !empty($options['generate_tints']),
                'count' => $options['tint_count'] ?? 6,
                'target' => ['r' => 255, 'g' => 255, 'b' => 255],
            ],
        ];

        foreach ($variant_groups as $prefix => $group) {
            if (empty($group['enabled'])) {
                continue;
            }

            $count = min(10, max(4, (int) $group['count']));
            for ($i = 1; $i <= $count; $i++) {
                $amount = $i / ($count + 1);
                $mixed = [
                    'r' => round($base['r'] + ($group['target']['r'] - $base['r']) * $amount),
                    'g' => round($base['g'] + ($group['target']['g'] - $base['g']) * $amount),
                    'b' => round($base['b'] + ($group['target']['b'] - $base['b']) * $amount),
                    'a' => $base['a'],
                ];
                $variants[$prefix . '-' . $i] = $this->format_css_color($mixed, $base['a'] < 1 ? 'rgba' : 'hex');
            }
        }
