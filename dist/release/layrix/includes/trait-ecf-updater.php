...
trait ECF_Framework_Updater_Trait {
    ...
    private function legacy_plugin_basenames() {
        $basenames = [];

        if (defined('ECF_LEGACY_PLUGIN_FILE')) {
            $legacy_basename = plugin_basename(ECF_LEGACY_PLUGIN_FILE);
            if ($legacy_basename !== '' && $legacy_basename !== $this->plugin_basename()) {
                $basenames[] = $legacy_basename;
            }
        }

        $default_legacy = $this->update_target_plugin_slug() . '/Layrix.php';
        if ($default_legacy !== $this->plugin_basename()) {
    ...
    private function update_target_plugin_slug() {
        $plugin_slug = trim((string) $this->plugin_slug());
        if ($plugin_slug !== '') {
            return $plugin_slug;
        }
    ...
    private function replace_plugin_basename_in_list(array $plugins, $replacement, array $candidates) {
        $updated = false;

        foreach ($plugins as $index => $plugin) {
            if (!in_array($plugin, $candidates, true)) {
                continue;
            }
    ...
    private function replace_plugin_basename_in_map(array $plugins, $replacement, array $candidates) {
        $updated = false;

        foreach ($candidates as $candidate) {
            if (!array_key_exists($candidate, $plugins)) {
                continue;
            }

            if (!array_key_exists($replacement, $plugins)) {
                $plugins[$replacement] = $plugins[$candidate];
            }

            unset($plugins[$candidate]);
            $updated = true;
        }
    ...
    public function migrate_legacy_plugin_basename() {
        $current = $this->plugin_basename();
        $legacy_basenames = $this->legacy_plugin_basenames();

        if (empty($legacy_basenames)) {
            return;
        }

        $active_plugins = (array) get_option('active_plugins', []);
        [$active_plugins, $active_updated] = $this->replace_plugin_basename_in_list($active_plugins, $current, $legacy_basenames);
        if ($active_updated) {
            update_option('active_plugins', $active_plugins);
        }

        $auto_update_plugins = (array) get_site_option('auto_update_plugins', []);
        [$auto_update_plugins, $auto_updated] = $this->replace_plugin_basename_in_list($auto_update_plugins, $current, $legacy_basenames);
        if ($auto_updated) {
        ...
        if (is_multisite()) {
            ...
            [$network_active_plugins, $network_updated] = $this->replace_plugin_basename_in_map($network_active_plugins, $current, $legacy_basenames);
            if ($network_updated) {
        ...
        $updated_transient = false;

        if (isset($transient->checked) && is_array($transient->checked)) {
            [$transient->checked, $checked_updated] = $this->replace_plugin_basename_in_map($transient->checked, $current, $legacy_basenames);
            $updated_transient = $updated_transient || $checked_updated;
        }

        if (isset($transient->response) && is_array($transient->response)) {
            [$transient->response, $response_updated] = $this->replace_plugin_basename_in_map($transient->response, $current, $legacy_basenames);
            $updated_transient = $updated_transient || $response_updated;
        }

        if (isset($transient->no_update) && is_array($transient->no_update)) {
            [$transient->no_update, $no_update_updated] = $this->replace_plugin_basename_in_map($transient->no_update, $current, $legacy_basenames);
            $updated_transient = $updated_transient || $no_update_updated;
        }

        if ($updated_transient) {
            set_site_transient('update_plugins', $transient);
        }
    }
    ...
    private function github_latest_release_api_url() {
        return $this->github_api_url('releases/latest');
    }
    ...
    private function normalize_release_version($version) {
        $version = trim((string) $version);
        return ltrim($version, "vV");
    }

    private function github_release_asset_candidates($version) {
        $version = $this->normalize_release_version($version);
        if ($version === '') {
            return [];
        }
        ...
        return [
            sprintf('%s-%s-transition.zip', $slug, $version),
            sprintf('%s-%s.zip', $slug, $version),
        ];
    }

    private function github_release_package_url($version) {
        $response = wp_remote_get($this->github_latest_release_api_url(), $this->github_request_args());
        ...
        $release_version = $this->normalize_release_version($body['tag_name'] ?? '');
        if ($release_version !== '' && $release_version !== $this->normalize_release_version($version)) {
            return '';
        }

        $assets = isset($body['assets']) && is_array($body['assets']) ? $body['assets'] : [];
        $candidates = $this->github_release_asset_candidates($version);
        ...
        foreach ($assets as $asset) {
            ...
            $prefix = $this->canonical_plugin_slug() . '-' . $this->normalize_release_version($version);
    ...
    private function build_github_update_payload($plugin_file, $installed_version) {
        ...
        return (object) [
            ...
            'slug' => $this->update_target_plugin_slug(),
            'plugin' => $plugin_file,
    ...
    public function rename_github_update_source($source, $remote_source, $upgrader, $hook_extra) {
        ...
        $target = trailingslashit($remote_source) . $this->update_target_plugin_slug();
        if ($source === $target) {
            return $source;
        }
    ...
    public function normalize_github_plugin_destination($response, $hook_extra, $result) {
        ...
        $expected = untrailingslashit(WP_PLUGIN_DIR . '/' . $this->update_target_plugin_slug());
