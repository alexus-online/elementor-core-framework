...
trait ECF_Framework_Native_Elementor_Data_Trait {
    ...
    /**
     * Reverse lookup: Variable-ID (e-gv-xxx) → Label (e.g. 'ecf-space-m').
     * Used by sync-conflict detection to compare what is currently in
     * Elementor's class registry against what Layrix's backend expects.
     */
    private function reverse_lookup_variable_label($var_id) {
        if (!is_string($var_id) || $var_id === '') return null;
        ...
        try {
            ...
            foreach ($collection->all() as $id => $variable) {
                if ((string) $id === $var_id && method_exists($variable, 'label')) {
                    return (string) $variable->label();
                }
            }
        } catch (\Throwable $e) {}
        return null;
    ...
    /**
     * Pull a single Layrix-managed prop value out of an Elementor class
     * variant's props array, normalized back to a token-label string
     * (e.g. 'ecf-space-m'). Knows about the padding-Dimensions sub-keys
     * (block-start/inline-end/block-end/inline-start) that Layrix uses
     * for its 4 separate padding props.
     */
    private function extract_class_prop_value(array $props, string $prop_key) {
        // Padding Pair-Keys: extract from Dimensions sub-key
        $padding_subkey_map = [
            'padding-block-start'  => 'block-start',
            'padding-block-end'    => 'block-end',
            'padding-inline-start' => 'inline-start',
            'padding-inline-end'   => 'inline-end',
            // Backward-Compat (alte Schema-Variante)
            'padding-block'        => 'block-start',
            'padding-inline'       => 'inline-end',
        ];
        if (isset($padding_subkey_map[$prop_key])) {
            $padding = $props['padding'] ?? null;
            if (!is_array($padding) || ($padding['$$type'] ?? '') !== 'dimensions') return null;
            $val = $padding['value'] ?? null;
            if (!is_array($val)) return null;
            return $this->size_ref_to_label($val[$padding_subkey_map[$prop_key]] ?? null);
        }
        if (in_array($prop_key, ['border-radius', 'font-size', 'max-width'], true)) {
            return $this->size_ref_to_label($props[$prop_key] ?? null);
        }
        if (in_array($prop_key, ['background-color', 'color', 'border-color'], true)) {
            $ref = $props[$prop_key] ?? null;
            if (!is_array($ref)) return null;
            $type = $ref['$$type'] ?? '';
            if ($type === 'global-color-variable') {
                return $this->reverse_lookup_variable_label((string) ($ref['value'] ?? ''));
            }
            if ($type === 'color') {
    ...
    private function size_ref_to_label($ref) {
        if (!is_array($ref)) return null;
        $type = $ref['$$type'] ?? '';
        ...
        if ($type === 'size') {
            $sv = $ref['value'] ?? null;
            if (is_array($sv) && isset($sv['size'], $sv['unit'])) {
                return $sv['size'] . $sv['unit'];
            }
        }
        return null;
    }
    ...
    /**
     * Detect divergences between Layrix-backend defaults and what is currently
     * stored in Elementor's Global-Classes-Registry for Layrix-managed classes.
     *
     * @param array|null $skip_props Optional list of "{class}.{prop}" keys
     *        to skip (e.g. props the user just changed in Layrix UI; those
     *        should propagate Backend → Elementor without prompting).
     * @return array List of [class, class_label, prop, prop_label, backend, elementor].
     */
    public function detect_class_sync_conflicts(?array $skip_props = null): array {
        $conflicts = [];
        if (!class_exists('\Elementor\Modules\GlobalClasses\Global_Classes_Repository')) {
            return $conflicts;
        }
        $schema = $this->layrix_class_defaults_schema();
        if (empty($schema)) return $conflicts;
        ...
        // Real-Class-ID-Lookup (matchen Klasse über Label statt deterministischer ID)
        $label_to_id = [];
        ...
        foreach ($schema as $class_name => $cls_def) {
            $class_id = $label_to_id[strtolower($class_name)] ?? ('g-ecf-' . substr(md5($class_name), 0, 10));
            if (!isset($items[$class_id])) continue;
            $variants = $items[$class_id]['variants'] ?? [];
            if (!is_array($variants)) continue;

            $default_variant = null;
            foreach ($variants as $v) {
                $bp = $v['meta']['breakpoint'] ?? 'desktop';
                $state = $v['meta']['state'] ?? null;
                if ($bp === 'desktop' && $state === null) {
                    $default_variant = $v;
                    break;
                }
            }
            if (!$default_variant) continue;
            $el_props = is_array($default_variant['props'] ?? null) ? $default_variant['props'] : [];

            foreach ($cls_def['props'] as $prop_key => $prop_def) {
                $skip_key = $class_name . '.' . $prop_key;
                if (is_array($skip_props) && in_array($skip_key, $skip_props, true)) continue;
                if (isset($prop_def['editable']) && $prop_def['editable'] === false) continue;

                $backend_value = $this->resolve_layrix_class_default($class_name, $prop_key, $settings);
                if ($backend_value === '' || $backend_value === null) continue;

                $elementor_value = $this->extract_class_prop_value($el_props, $prop_key);
                ...
                if ((string) $elementor_value !== (string) $backend_value) {
                    $conflicts[] = [
                        ...
                        'prop'        => $prop_key,
                        'prop_label'  => (string) ($prop_def['label'] ?? $prop_key),
                        'backend'     => (string) $backend_value,
                        'elementor'   => (string) $elementor_value,
                    ];
                }
            }
        }
