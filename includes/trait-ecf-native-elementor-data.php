...
trait ECF_Framework_Native_Elementor_Data_Trait {
     ...
     * Used by sync-conflict detection to compare what's in Elementor's
     * class registry against what Layrix's backend currently expects.
     */
    ...
    /**
     * Extract a single Layrix-managed prop value from an Elementor class variant's
     * props array, normalized back to the same form Layrix-backend uses (token
     * label like 'ecf-space-m' or a literal CSS value).
     *
     * Returns null when the prop isn't set in the variant or can't be read.
     */
    private function extract_class_prop_value(array $props, string $prop_key) {
        // Padding is stored as a single Dimensions prop; pull block-start (block)
        // or inline-end (inline) since Layrix writes symmetric values.
        if ($prop_key === 'padding-block' || $prop_key === 'padding-inline') {
            ...
            $sub_key = $prop_key === 'padding-block' ? 'block-start' : 'inline-end';
            return $this->size_ref_to_label($val[$sub_key] ?? null);
        }
    ...
    /**
     * Detect divergences between Layrix-backend defaults and what's currently
     * stored in Elementor's Global-Classes-Registry for Layrix-managed classes.
     *
     * @param array|null $skip_props Optional list of "{class}.{prop}" keys to
     *        skip (e.g. props the user just changed in the Layrix UI — those
     *        should propagate Backend → Elementor without prompting).
     * @return array List of [class, prop, prop_label, backend, elementor].
     */
    public function detect_class_sync_conflicts(?array $skip_props = null): array {
        ...
        foreach ($schema as $class_name => $cls_def) {
            $class_id = 'g-ecf-' . substr(md5($class_name), 0, 10);
            if (!isset($items[$class_id])) continue;
            $variants = $items[$class_id]['variants'] ?? [];
            if (!is_array($variants)) continue;

            // Compare against the default variant (desktop, no state).
            ...
            foreach ($cls_def['props'] as $prop_key => $prop_def) {
                ...
                if ($elementor_value === null) continue; // not set yet — first sync will write it
