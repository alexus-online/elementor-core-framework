...
jQuery(function($){
  ...
  function applySettingsPayload(preset, $button, messages) {
    ...
    // Snapshot aktuellen Zustand für Undo
    try {
  ...
  function renderShadowPreview() {
    ...
    // keep sidebar subnav in sync whenever the active shadow changes
    if (typeof syncShadowSubnavActive === 'function') {
      syncShadowSubnavActive();
    }
  }

  // ── Token focus mode (drill-down into single token) ────────────
  function enterTokenFocus(type, slug) {
    var $panel = $('.ecf-panel[data-panel="tokens"]');
    var $card  = type === 'color'
      ? $panel.find('[data-ecf-layout-item="tokens-colors"]')
      : $panel.find('[data-ecf-layout-item="tokens-radius"]');
    ...
    // Open color detail panel if color
    if (type === 'color') {
    ...
    // Set focus mode on panel shell
    $panel.find('.ecf-card.ecf-panel-shell').attr('data-ecf-token-focus', type);

    // Inject back bar if not yet present
    if (!$card.find('.ecf-token-focus-bar').length) {
      var tokenName = type === 'color' ? '--ecf-color-' + slug : '--ecf-radius-' + slug;
      var $bar = $('<div class="ecf-token-focus-bar">'
        + '<button type="button" class="ecf-token-focus-back" data-ecf-focus-back>'
        + '<span class="dashicons dashicons-arrow-left-alt2"></span>'
        + (type === 'color' ? 'Alle Farben' : 'Alle Radius')
        + '</button>'
        + '<span class="ecf-token-focus-label">' + escapeHtml(tokenName) + '</span>'
        + '</div>');
  ...
  function buildShadowSubnav() {
    ...
    items.forEach(function(item) {
      ...
      html += '<button type="button"'
        + ' class="ecf-nav-subnav__item' + (isActive ? ' is-active' : '') + '"'
        + ' data-ecf-subnav-shadow="' + item.slug + '"'
        + '>'
        + '<span class="ecf-nav-subnav__dot"></span>'
        + '<span class="ecf-nav-subnav__label">' + item.name + '</span>'
        + '<span class="ecf-nav-subnav__token">--ecf-shadow-' + item.slug + '</span>'
        + '</button>';
    });
    $subnav.html(html);
  }
  ...
  function enterShadowFocus(slug) {
    ...
    // Enable focus mode on preview card
    $preview.attr('data-ecf-shadow-focus-mode', slug);

    // Highlight matching editor row
    ...
    // Show focus bar if not there
    var $bar = $preview.find('.ecf-shadow-focus-bar');
    if (!$bar.length) {
      $bar = $('<div class="ecf-shadow-focus-bar">'
        + '<button type="button" class="ecf-token-focus-back" data-ecf-shadow-focus-back>'
        + '<span class="dashicons dashicons-arrow-left-alt2"></span>Alle Schatten'
        + '</button>'
        + '<span class="ecf-token-focus-label">--ecf-shadow-' + escapeHtml(slug) + '</span>'
        + '</div>');
  ...
  function enterSpacingFocus(step) {
    ...
    // inject or update focus bar
    var $bar = $preview.find('.ecf-spacing-focus-bar');
    if (!$bar.length) {
      $bar = $('<div class="ecf-spacing-focus-bar">'
        + '<button type="button" class="ecf-token-focus-back" data-ecf-spacing-focus-back>'
        + '<span class="dashicons dashicons-arrow-left-alt2"></span>Alle Schritte</button>'
        + '<span class="ecf-token-focus-label">--ecf-space-' + escapeHtml(step) + '</span>'
        + '</div>');
  ...
  // Toggle subnav accordion groups
  ...
  $(document).on('click', '[data-ecf-subnav-token]', function() {
    ...
    // If already focused on this token — exit focus mode
  ...
  function updateSaveButtonVisibility() {
    ...
    if (isNoSavePanel || !isDirty) {
      ...
      if ($save.prop('hidden')) {
        ...
        setTimeout(function() { $save.removeClass('ecf-save-entrance'); }, 400);
      }
      ...
      $group.sortable({
        ...
        start: function(event, ui) {
          ...
          if (isMasonryGroup) {
            ...
            clearMasonryLayoutToGroup($group);
            ui.placeholder.height(ui.item.outerHeight());
            // Refresh cached item positions after masonry reflow and force vertical sort
  ...
  function buildTypographySubnav() {
    ...
    items.forEach(function(item) {
      html += '<button type="button"'
        + ' class="ecf-nav-subnav__item' + (item.tab === activeTab ? ' is-active' : '') + '"'
        + ' data-ecf-subnav-typography="' + item.tab + '"'
        + '>'
        + '<span class="ecf-nav-subnav__dot"></span>'
        + '<span class="ecf-nav-subnav__label">' + item.label + '</span>'
        + '</button>';
    });
    $subnav.html(html);
  }
  ...
  // ── Spacing sidebar sub-navigation ─────────────────────────────
  function buildSpacingSubnav() {
    var $subnav = $('[data-ecf-subnav="spacing"]');
    if (!$subnav.length) return;

    var steps = getSpacingSteps ? getSpacingSteps() : [];
    if (!steps.length) { $subnav.html(''); return; }

    var html = '';
    steps.forEach(function(step) {
      html += '<button type="button" class="ecf-nav-subnav__item" data-ecf-subnav-spacing="' + escapeHtml(step) + '">'
        + '<span class="ecf-nav-subnav__dot"></span>'
        + '<span class="ecf-nav-subnav__label">' + escapeHtml(step) + '</span>'
        + '<span class="ecf-nav-subnav__token">--ecf-space-' + escapeHtml(step) + '</span>'
        + '</button>';
    });
    $subnav.html(html);
  }
  ...
  // ── Base Settings (components) sidebar sub-navigation ──────────
  ...
  function buildComponentsSubnav() {
    ...
    componentsSubnavItems.forEach(function(item) {
      html += '<button type="button"'
        + ' class="ecf-nav-subnav__item' + (item.tab === activeTab ? ' is-active' : '') + '"'
        + ' data-ecf-subnav-components="' + item.tab + '"'
        + '>'
        + '<span class="ecf-nav-subnav__dot"></span>'
        + '<span class="ecf-nav-subnav__label">' + item.label + '</span>'
        + '</button>';
  ...
  $(document).on('click', '[data-ecf-subnav-typography]', function() {
    ...
    if (!tab) return;
    // Switch to typography panel first if not active
  ...
  updateBaseBodyTextSizeWarning();
  syncTypographyFontCardSummaries();
  initUiWidgets($, { restUrl: restUrl, restNonce: restNonce, buildVariableSyncHash: buildVariableSyncHash, buildSettingsPayloadFromForm: buildSettingsPayloadFromForm, switchPanel: switchPanel });
